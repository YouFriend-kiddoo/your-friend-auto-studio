...
print(f"KEYS: GEMINI={bool(API_KEY)} YT={bool(YT_CLIENT_ID)}")

client = genai.Client(api_key=API_KEY) if API_KEY else None

def get_story():
    topics = ["Lion and Turtle best friends","Honest woodcutter magical axe","Greedy crow sharing","Little turtle race","Kind girl helps lion"]
    topic = random.choice(topics)
    prompt = f"Write 350 word cute kids moral story about: {topic}. Title: short title with emoji. Story: simple English, dialogues, happy ending, Moral at end."
    ...
    if not text:
        # Fallback story if all APIs fail
        text = f"Title: The Kind Turtle 🐢\nStory: Once there was a kind turtle named Timmy. Timmy helped all animals in forest. One day lion was stuck. Timmy called friends and helped lion. Lion thanked turtle. All became friends. Moral: Kindness wins always."
...
def upload(video_path, title, story):
    ...
    print(f"Init {init.status_code}: {init.text[:400]}")
